...
class geolocation:
  ...
  def geogoogle(self, addr, api_key):
        url = "https://maps.googleapis.com/maps/api/geocode/json?address=%s&key=%s" % (urllib.parse.quote(addr), urllib.parse.quote(api_key))
        try:
  ...
  def geobing(self, addr, api_key):
       url = "http://dev.virtualearth.net/REST/v1/Locations?q=%s&key=%s" % (urllib.parse.quote(addr), urllib.parse.quote(api_key))
       try:
         data = urllib.request.urlopen(url).read()
  ...
  def geonames(self, addr, api_key):
       url = "http://api.geonames.org/search?q=%s&maxRows=1&username=%s" % (urllib.parse.quote(addr), urllib.parse.quote(api_key))
       try:
         data = urllib.request.urlopen(url).read()
       except:
         return "Error"
