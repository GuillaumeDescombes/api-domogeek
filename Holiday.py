...
class jourferie:
  ...
  def dateliste(self,c, sep='/'):
      """Transforme une date chaine 'j/m/a' en une date liste [j,m,a]"""
      j, m, a = c.split(sep)
      return [int(j), int(m), int(a)]
  
  def datechaine(self,d, sep='/'):
      """Transforme une date liste=[j,m,a] en une date chaine 'jj/mm/aaaa'"""
      return ("%02d" + sep + "%02d" + sep + "%0004d") % (d[0], d[1], d[2])
  
  
  def jourplus(self,d, n=1):
      """Donne la date du nieme jour suivant d=[j, m, a] (n>=0)"""
      j, m, a = d
  ...
  def jourmoins(self,d, n=-1):
      """Donne la date du nieme jour precedent d=[j, m, a] (n<=0)"""
      j, m, a = d
      fm = [0,31,28,31,30,31,30,31,31,30,31,30,31]
      if (a%4==0 and a%100!=0) or a%400==0:  # bissextile?
          fm[2] = 29
  ...
  def joursferies(self,an, sd=0, sep='/'):
      """Liste des jours feries France en date-chaine de l'annee an (nb entier). 
           sd=0 (=defaut): tous les jours feries. 
           sd=1: idem sans les sammedis-dimanches. 
           sd=2: tous + les 2 jours feries supplementaires d'Alsace-Moselle. 
           sd=3: idem sd=2 sans les samedis-dimanches"""
