...
class schoolcalendar:
  def getschoolcalendar(self, zone):
    ...
    if zoneok not in ["A","B","C"]:
     ...
     try:
       ics = urllib.request.urlopen(URL)
     ...
     for event in cal.walk('vevent'):
        ...
        startdecode = time.strptime(convertstart.decode('utf-8'), '%Y%m%d')
        end =  event.get('dtend')
        ...
        if end:
           ...
           enddecode = time.strptime(convertend.decode('utf-8'), '%Y%m%d')
  ...
  def isschoolcalendar(self,zone,day,month,year):
    ...
    if zoneok not in ["A","B","C"]:
     ...
     for event in cal.walk('vevent'):
        ...
        convertstart = start.to_ical()
        stringconvertstart= str(convertstart)
        startdecode = time.strptime(convertstart.decode('UTF-8'), str('%Y%m%d'))
        end =  event.get('dtend')
        summary = event.get('summary')
        yearstart = startdecode[0]
        monthstart = startdecode[1]
        ...
        d3 = None
        description = summary.to_ical().decode('utf-8')
        if str(year).encode('UTF-8') in convertstart:
